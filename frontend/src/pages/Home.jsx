  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setErrorMessage(
          "Unable to load tracked products. The backend might be unreachable."
        );
        setPageState("error");
      }
    };
  ...
  const handleUrlSubmit = async (e) => {
    ...
    try {
      ...
      if (data && data.status === "active") {
        const productPid = data.originalResponse.productPid;
        if (productPid) navigate(`/products/${productPid}`);
        else {
          setTrackState("idle");
